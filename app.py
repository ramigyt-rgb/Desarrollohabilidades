...
from ppa_assets import get_ppa_figure
...
def page_exams(user, profile, skill):
    ...
    def show_figure(q):
        ...
        if page:
            img_data = get_ppa_figure(page)
            if img_data:
                with st.expander(f"Ver Figura {fig} · Anexo ANAC", expanded=True):
                    st.image(img_data, caption=f"Figura {fig} · Anexo PPA", use_container_width=True)
    ...
    with tabs[2]:
        ...
        st.caption("Las figuras ANAC necesarias están incorporadas dentro de la app; no necesitás subir carpetas adicionales.")
...
# Antes de completar el setup, el menú lateral debe seguir siendo funcional.
# Se usa una habilidad temporal (la elegida en onboarding o Aviación por defecto)
# sin guardar nada hasta que el usuario confirme el setup.
if not profile.get("active_skill"):
    temp_profile = dict(profile)
    temp_profile["active_skill"] = st.session_state.get("onboard_skill", "Aviación")
    temp_profile["daily_minutes"] = int(temp_profile.get("daily_minutes") or 30)
    temp_profile["target"] = temp_profile.get("target") or ""
    temp_profile["experience"] = temp_profile.get("experience") or "Principiante"

    sidebar(user, temp_profile)

    if st.session_state.nav == "Inicio":
        onboarding(user)
    else:
        temp_skill = temp_profile["active_skill"]
        pages[st.session_state.nav](user, temp_profile, temp_skill)
    st.stop()
